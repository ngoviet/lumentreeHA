...
def parse_mqtt_payload(ph: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if is_cell_data:
            ...
            # Battery current (inverted)
            bat_curr = rr("BATTERY_CURRENT", True, 0.01)
            if bat_curr is not None:
                parsed_data[KEY_BATTERY_CURRENT] = -bat_curr
            ...
            if bp_signed is not None:
                parsed_data[KEY_BATTERY_POWER] = abs(bp_signed)
                parsed_data[KEY_BATTERY_STATUS] = (
                    "Charging" if bp_signed < 0 else "Discharging"
                )
            else:
    ...
    if parsed_data:
        ...
        _LOGGER.warning(f"No data parsed from: {resp_hex[:60]}...")
        return None
