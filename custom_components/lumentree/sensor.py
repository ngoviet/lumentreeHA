...
async def async_setup_entry(
    ...
    _LOGGER.info(f"Adding {len(REALTIME_SENSOR_DESCRIPTIONS)} real-time sensors for {device_sn}")
        ...
        # Add synthetic sensor: total load power = load + AC out
    entities_to_add.append(
        TotalLoadPowerSensor(
            hass,
            entry,
            device_info,
...
class TotalLoadPowerSensor(LumentreeMqttSensor):
    @callback
    def _handle_update(self, data):
        load = data.get(KEY_LOAD_POWER) or 0
        ac = data.get(KEY_AC_OUT_POWER) or 0
        new_value = load + ac
