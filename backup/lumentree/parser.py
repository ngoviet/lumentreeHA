...
def _parse_battery_cells(db: bytes) -> Optional[Dict[str, Any]]:
    ...
    if nc > 0:
        ...
        if _LOGGER.isEnabledFor(logging.DEBUG):
            _LOGGER.debug("Parsed cells: %s", res)
        return res
    else:
...
def parse_mqtt_payload(ph: str) -> Optional[Dict[str, Any]]:
    if _LOGGER.isEnabledFor(logging.DEBUG):
        _LOGGER.debug("Parsing: %s...", ph[:100])
    ...
    try:
        ...
        if bc==expected_cell_bytes and len(db)==expected_cell_bytes: 
        ...
        elif (bc==expected_main_bytes and len(db)==expected_main_bytes) or (bc==expected_main_bytes_extended and len(db)==expected_main_bytes_extended):
            is_cell=False
            ...
            _LOGGER.error(f"Unrec len ({len(db)}/{bc}) for 95/50 regs. Expected: {expected_main_bytes} or {expected_main_bytes_extended} bytes for main data, {expected_cell_bytes} for cell data.")
            return None
        ...
        if is_cell:
            ...
            if _LOGGER.isEnabledFor(logging.DEBUG):
                _LOGGER.debug("Parsed main data final: %s", parsed_data)
